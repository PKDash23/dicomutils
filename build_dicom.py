...
parser.add_argument('--meterset', dest='meterset', default='1.0',
                    help="""Set the beam meterset weight, either as a single number for all beams
                    or as [1;2;3;...] to specify the meterset weight for all beams in an RTPLAN.""")
    ...
    for series in study:
        ...
        if series.modality == "CT":
            ...
            if all(d.isdigit() for d in series.patient_support_angles):
                ...
                patient_support_angles = \
                    [int(b) for b in series.patient_support_angles.lstrip('[').rstrip(']').split(";")]
            ...
            for i in range(nbeams):
                rp.build_beam(gantry_angle=gantry_angles[i],
                              meterset=meterset[i],
                              collimator_angle=collimator_angles[i],
                              patient_support_angle=patient_support_angles[i],
                              table_top=table_top,
                              table_top_eccentric=table_top_eccentric)
